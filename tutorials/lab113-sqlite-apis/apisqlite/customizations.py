...
def prompt_type():
    while True:
        ...
        resource_type = input("What would you like to search for?\n" + "\n".join(valid_types) + "\n>").strip().lower()
        if resource_type in valid_types:
            return resource_type

def prompt_year():
    while True:
        year = input("What is the release year? ").strip()
        if year.isnumeric() and int(year) > 0:
            return int(year)

# search for all movies containing string
def movielookup(mykey, searchstring, resource_type=None, release_year=None):
    ...
    try:
        ...
        if resource_type:
            api += f"&type={resource_type}"
        if release_year:
            api += f"&y={release_year}"
    ...
    while True:
        ...
        while answer == "":
            ...
            ** Returned data will be written into the local database **
            ...
            3) Search for Movies Containing String, by release year
            4) Search for Movies Cmontaining String, by type and release year
            99) Exit""")
        ...
        if answer in ["1", "2", "3", "4"]:
            ...
            if answer == "1":
                ...
                resource_type = prompt_type()
                resp = movielookup(mykey, searchstring, resource_type=resource_type)
            elif answer == "3":
                release_year = prompt_year()
                resp = movielookup(mykey, searchstring, release_year=release_year)
            elif answer == "4":
                release_year = prompt_year()
                resource_type = prompt_type()
                resp = movielookup(mykey, searchstring, resource_type=resource_type, release_year=release_year)
            if resp:
                # display the results
